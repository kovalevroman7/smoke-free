...
export default function StatsTab({
  data, statsPeriod, setStatsPeriod,
  selectedDay, setSelectedDay,
  openSwipeIndex, setOpenSwipeIndex,
  onStartEditing, onDeleteByIndex
}) {
  ...
  const prevPeriodTotal = prevPeriodDays.reduce(
    (sum, day) => sum + data.cigarettes.filter(t => getDateKey(t) === day).length, 0
  )
  ...
  return (
    <div className="stats-card">
      ...
      {(data.goals || []).length > 0 && (() => {
        ...
        const trailingEmpty = isMonth ? (6 - ((new Date(periodDays[periodDays.length - 1]).getDay() + 6) % 7)) : 0
        return (
          <div className={`goals-week-block ${isMonth ? 'month' : ''}`}>
            ...
            <div className="goals-week-days">
              {isMonth
                ? weekdayLabels.map(wd => <div key={wd} className="goals-week-day-label">{wd}</div>)
                : periodDays.map(day => <div key={day} className="goals-week-day-label">{getDayOfWeek(day)}</div>)
              }
            </div>
            {goals.map(goal => {
              ...
              return (
                <div key={goal.id} className="goals-week-goal">
                  ...
                  <div className="goals-week-cells">
                    {Array.from({ length: leadingEmpty }, (_, i) => <div key={`lead-${i}`} className="goals-week-cell empty" />)}
                    {periodDays.map(day => {
                      if (day < goalStartKey) return <div key={day} className="goals-week-cell na">·</div>
                      if (day > todayKey) return <div key={day} className="goals-week-cell empty" />
                    ...
                    {Array.from({ length: trailingEmpty }, (_, i) => <div key={`tail-${i}`} className="goals-week-cell empty" />)}
                  </div>
                </div>
              )
            })}
            ...
            {(() => {
              ...
              return hourly.map(({ hour, count }) => (
                <div key={hour} className="hourly-bar-wrapper">
                  ...
                  <div className="hourly-bar" style={{ height: `${(count / maxHourly) * 60}px` }} title={`${hour}:00 - ${count} шт`} />
                  ...
                  {dayCigarettes.map((time, i) => {
                    ...
                    return (
                      <SwipeableItem
                        ...
                        onEdit={() => { onStartEditing(time, originalIndex); setOpenSwipeIndex(null) }}
